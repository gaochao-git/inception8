...
#include "sql/inception/inception.h"  // inception hooks
  ...
  */
  ...
  switch (command) {
    case COM_INIT_DB: {
      ...
      if (inception::handle_use_db(thd, tmp.str, tmp.length)) break;
...
int mysql_execute_command(THD *thd, bool first_level) {
  ...
  */
  ...
  /* Inception hook: intercept parsed SQL during active inception session */
  if (inception::intercept_statement(thd)) {
    goto finish;
  }
...
void dispatch_sql_command(THD *thd, Parser_state *parser_state) {
  ...
  /* Inception hook: magic comments and special commands */
  if (inception::before_parse(thd)) return;
  ...
  /* Inception: record parse error and continue multi-statement loop */
  if (err && inception::handle_parse_error(thd, &parser_state->m_lip)) return;
